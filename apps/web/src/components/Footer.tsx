...
export default function Footer({ tpScore = '4.8', tpTotal = 500 }) {
    ...
    return (
        <footer className="pt-20 md:pt-24 pb-32 md:pb-16 border-t border-white/5 bg-black/40 relative overflow-hidden">
            {/* Trustpilot Banner */}
            <div className="max-w-7xl mx-auto px-6 mb-20">
                <div className="p-8 md:p-12 rounded-[2.5rem] bg-gradient-to-br from-[#121212] to-[#0A0A0A] border border-white/10 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden group">
                    ...
                    <div className="flex flex-col items-center md:items-start text-center md:text-left">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-2xl font-black uppercase italic tracking-tight">Trustpilot</span>
                        </div>
                        <div className="flex gap-1.5 mb-2">
                            {[1, 2, 3, 4, 5].map(i => <Star key={i} className="w-6 h-6 text-[#00B67A] fill-[#00B67A]" />)}
                        </div>
                        <p className="text-gray-400 font-medium">{t('reviewsText')}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full md:w-auto">
                        {[
                            ...
                            <div key={i} className="px-6 py-4 rounded-2xl bg-white/[0.03] border border-white/5 flex flex-col gap-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12 mb-16 md:mb-20">
                    <div className="sm:col-span-2 lg:col-span-2">
                        ...
                        <div className="flex flex-wrap items-center gap-6 opacity-30 grayscale hover:opacity-100 hover:grayscale-0 transition-all duration-500">
                            <div className="h-6 w-auto bg-white/20 px-3 rounded text-[10px] flex items-center font-bold text-white tracking-widest italic">VISA</div>
                            <div className="h-6 w-auto bg-white/20 px-3 rounded text-[10px] flex items-center font-bold text-white tracking-widest italic">MASTERCARD</div>
                            <div className="h-6 w-auto bg-white/20 px-3 rounded text-[10px] flex items-center font-bold text-white tracking-widest italic">STRIPE</div>
                            <div className="h-6 w-auto bg-white/20 px-3 rounded text-[10px] flex items-center font-bold text-white tracking-widest italic">MP</div>
                        </div>
                    </div>

                    <div>
                        <h4 className="text-white font-bold mb-6 text-sm uppercase tracking-widest">{t('legal')}</h4>
